...
split_method="random"
dataset_name="cyanine" # 可选外部数据集：cyanine、xanthene
...
do
    ...
    echo "Predicting external fold ${fold} on CUDA device ${cuda_id}"
    CUDA_VISIBLE_DEVICES=${cuda_id} python predict.py \
        ...
        --metric r2 \
        --split splits \
        --results_dir "results/${split_method}/${dataset_name}_fold${fold}/${task_name}"
done
